...
function myWay(certainPos) {
	...
	/*if (sourceVertex <= 132) {
		...
		$.ajax({
			...
			success: function(result) {
				if((!isNaN(sourceVertex) || sourceVertex == '您当前的位置') && !isNaN(finalVertex)) {
			...
			success: function(result) {
				if((!isNaN(sourceVertex) || sourceVertex == '您当前的位置') && !isNaN(finalVertex)) {
					if(sourceVertex == '您当前的位置') {
						...
						sourceVertex = result.features[sourceVertex - 133].properties.Rid;
					}
				}
			},
			dataType: 'json',
		...
		$.ajax({
			...
			success: function(result) {
				if((!isNaN(sourceVertex) || sourceVertex == '您当前的位置') && !isNaN(finalVertex)) {
					...
					document.getElementById("finalVertex").value = finalVertex;
					//转换id和rid
					finalVertex = result.features[finalVertex - 133].properties.Rid;
				}
			},
			dataType: 'json',
			//设置ajax为同步执行
	...
	//2.看路径中是否有中转点
	if($.inArray(237, path) != -1) {
		//判断是否为中转点或只是路过
		if(path[$.inArray(237, path) - 1] <= 111 && path[$.inArray(237, path) + 1] > 111) {
			transitPoint = 237;
		}
	} else if($.inArray(238, path) != -1) {
		if(path[$.inArray(238, path) - 1] <= 111 && path[$.inArray(238, path) + 1] > 111) {
			transitPoint = 238;
		}
	} else if($.inArray(239, path) != -1) {
		if(path[$.inArray(239, path) - 1] <= 111 && path[$.inArray(239, path) + 1] > 111) {
			transitPoint = 239;
		}
	} else if($.inArray(240, path) != -1) {
		if(path[$.inArray(240, path) - 1] <= 111 && path[$.inArray(240, path) + 1] > 111) {
			transitPoint = 240;
		}
	}
	//3.如果有中转点则打成两个数组,否则为同层导航
	if (transitPoint) {
		var path1 = path.slice(0, $.inArray(transitPoint, path) + 1);
		var path2 = path.slice($.inArray(transitPoint, path), path.length);
	} else {
		if (path[0] <= 111) {
			//在第一层
			path1 = path;
	...
	if(path1) {
		...
		$.ajax({
			url: "./data/1-circle.geojson",
			success: function(result) {
				if(!isNaN(sourceVertex) && !isNaN(finalVertex)) {
					var pathPos = [];
					var pos;
					for(i = 0; i < path1.length; i++) {
						if (result.features[path1[i]]) {
							var pos = result.features[path1[i]].properties.center;
							pathPos[i] = pos;
						} else {
							$.ajax({
								url: "./data/1-elevator.geojson",
								success: function(result) {
									var pos = result.features[path1[i]-237].properties.center;
									pathPos[i] = pos;
								},
								dataType: 'json',
								//设置ajax为同步执行
			...
			success: function(result) {
				if(!isNaN(sourceVertex) && !isNaN(finalVertex)) {
					...
					for(i = 0; i < path2.length; i++) {
						if (result.features[path2[i]-112]) {
							var pos = result.features[path2[i]-112].properties.center;
							pathPos[i] = pos;
						} else {
							$.ajax({
								url: "./data/2-elevator.geojson",
								success: function(result) {
									var pos = result.features[path2[i]-237].properties.center;
									pathPos[i] = pos;
								},
								dataType: 'json',
								//设置ajax为同步执行
					...
					if (document.querySelector('.on').innerHTML == "2F") {
						//0.清理原先图层
						if(lineLayer2){
							map.removeLayer(lineLayer2);
						}
						//1.给起点终点加点标注
						var startPos = pathPos[0];
						startMarkerOverlay.setPosition(startPos);
	...
	//4.用findpath方法得到路径图层并在其中判断是否显示
	//5.点击楼层按钮判断是否有路径图层，如果有便加入地图，同时将起终点覆盖物加上。*/
				...
				for(i = 0; i < result.ibeacons.length; i++) {
					if(result.ibeacons[i].minor == minDisBeacon.minor) {
						...
						for(j = 0; j < minDisBeacon.adjacentbeacons.length; j++) {
							...
							if(arrAdjacentBeacons[j] == secDisBeacon.minor) {
								inArrayFlag1 = true;
							} else if(arrAdjacentBeacons[j] == beaconsArr[2].minor) {
								inArrayFlag2 = true;
							}
						}
						if(inArrayFlag1) {
